...
void *mg_mmap(FILE *fp, size_t size) {
  void *p = mmap(NULL, size, PROT_READ, MAP_PRIVATE, _fileno(fp), 0);
...
static void forward_put_data(struct connection *conn) {
  ...
  int n = _write(conn->endpoint.fd, io->buf, k);   // Write them!
  if (n > 0) {
    iobuf_remove(io, n);
    conn->cl -= n;
  }
...
static void transfer_file_data(struct connection *conn) {
  ...
  // Do not send anyt
  n = _read(conn->endpoint.fd, buf, conn->cl < (int64_t) sizeof(buf) ?
